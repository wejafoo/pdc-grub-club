...
export class VersionUpdateComponent implements OnInit, OnDestroy {
	...
	calcTotals		(logMessage: string) {		// calcTotals (logMessage: string, event?: string, host?: string) {
	...
	dropGuest		(guestDrop:	CdkDragDrop<any[]>, evt: string) {		// unassigned->host / host->host
		...
		console.log( 'host or guest data?', toHost );
		if (fromHst === toHst) {
			console.log('PLAN:', this.sched, '\nSUMMARY:', this.summary);
	...
	unDropGuest		(guestDrop:	CdkDragDrop<any[]>, evt: string) {
		...
		const fromIdx	= guestDrop.previousIndex;
		if ( fromHst !== toHst ) {
			if (this.debug)  console.log( 'guest moved:', fromHst, '--->', toHst );
	...
	rePair			(evt: string, fromHost: string, toHost: string, gst: string) {
		if (this.debug)  console.log('rePair()\tevent:', evt, 'guest:', gst, 'from:', fromHost, 'to:', toHost);
