...
export class DialogContentComponent {
	...
	constructor(@Inject(MAT_DIALOG_DATA) public data: string) {
		this.results = JSON.stringify(data['aGs'])
	}
}
...
export class ScheduleComponent implements OnInit {
	...
	ngOnInit		()					{
		...
		this.presbies = this.presby.getPresbies();
		if (this.presbies === undefined) {
			this.router.navigate(['/plan', 's', this.planId]).then()
		} else { this.actives = this.presbies.filter(presby => presby.isActive)}
		this.ver.id++;
		this.sched = {
		...
		for (const a1 of this.actives) {
			...
			for (const a2 of this.actives) {
				if (a1 !== a2) { pairs[a1.key].push(a2.key)}
		}}
	...
	autoAllocate	(ctx: string)		{
		let events: string[];
		if (ctx === 'all') {
			events = Object.keys(this.unHs)} else { events = [ctx]
		}
		events.every(event =>
			this.unHs[event].sort(() => Math.random() - 0.5)
	...
	autoAssign		(ctx: string)		{
		...
		for (const evt of events) {
			...
			this.unGs[evt].sort(() => Math.random() - 0.5);			// Todo: refactor how "random" auto-assignment works // large groups first... for now // this.unGs[evt].sort((a: {guests: string[]},b: {guests: string[]}) => b.guests.length - a.guests.length);
			while (
				...
				for (const hst of hosts) {
					if (hst === nextHost) {
						if ( this.assignGuest(evt, hst, nextGuest) ) { break } else {
							skipCnt++;
	}}}}}}
	calcTotals		(logMsg: string)	{
		console.log(logMsg);
		for ( const evt of this.events ) {
			...
			this.summary[evt].overAllocatedSeats =
				this.summary[evt].allocatedSeatCnt - this.summary[evt].rsvps;
			...
			for (const hst in this.aGs[evt]) {
				...
				} else { this.summary[evt][hst].unAssignedSeatCnt = 0 }
		...
		for (const active of this.actives) {
			...
			for (const hosting of hostings) {
				...
				if (hostingObj.event === evt) {
					...
					hostingObj.hostKey =
						activeObj.last	+ '-' + activeObj.id	+ '-' +
						activeObj.seats	+ '-' + activeObj.guests.length;
	...
	assignGuest		(evt: string, hst: string, gst: string, guestDrop?: CdkDragDrop<any[]>): boolean {
		...
		if (hst === gst) {
			...
			if (fromIdx < 0 ) {
				for (const [host/*,hostArray*/] of Object.entries(this.aGs[evt])) {
					const oldIdx = this.aGs[evt][host].findIndex(
						guest => guest.guestKey === hst
					)
					if (oldIdx >= 0) {
						const newGuestIdx = this.aGs[evt][hst].push(
							...this.aGs[evt][host].splice(oldIdx, 1)
						) - 1;
				...
				const newGuestIdx = this.aGs[evt][hst].push(
					...this.unGs[evt].splice(fromIdx, 1)
				) - 1;
				this.aGs[evt][hst][newGuestIdx].isDisabled = true;
				success = true;
			}
			...
			if (guestDrop) {
				...
				if (
					this.aGs[evt][hst].every((aG: {guestKey: string}) =>
						this.summary.pairs[gst].includes(aG.guestKey))
				) {
					let fromHst: string;
					let toIdx: number;
					let fromIdx = this.unGs[evt].findIndex(
					...
					if (fromIdx < 0) {
						...
						for (const host of this.aGs[evt]) {
							...
							fromIdx	= this.aGs[evt][host.hostKey].findIndex(
								(guest: any) => guest.guestKey === gst
							);
							if (this.debug) { console.log(
								'\tChecking' + host.hostKey, '>', gst
							)}
							if (fromIdx > 0) {
								if (this.debug) { console.log(
									'\t\tFOUND IT:', host.hostKey + fromIdx
								)}
								toIdx = this.aGs[evt][hst].push(
									...this.aGs[evt][fromHst].splice(fromIdx, 1)
								)
	...
	dropGuest		(guestDrop:	CdkDragDrop<any[]>, evt: string) {
		const fromHst	= guestDrop.previousContainer.id;
		const toHst		= guestDrop.container.id;
	...
	unDropGuest		(guestDrop:	CdkDragDrop<any[]>, evt: string) {
		...
		if (fromHst !== toHst) {
			if (this.debug) {
				console.log('guest-plan-list moved:', fromHst, '--->', toHst)
			}
