...
export class VersionUpdateComponent implements OnInit, OnDestroy {
	...
	save() {
		console.log('!!! VersionUpdateComponent -> SAVING VERSION:', this.ver);
		
		localStorage.setItem('latest', JSON.stringify(this.ver));
	...
	calcTotals(logMessage: string) {		// calcTotals (logMessage: string, event-plan-list?: string, host?: string) {
		...
		}	// if (event-plan-list && host) { return this.summary[event-plan-list][host][logMessage] } else if ( event-plan-list ) { return this.summary[event-plan-list][logMessage] }
	}
	...
	autoAssign(ctx: string) {
		...
		for ( const evt of events ) {
			...
			while (
				...
				for (const hst of hosts) { if (hst === nextHost) { if (this.assignGuest(evt, hst, nextGuest)) { break } else {
					if (this.debug)  console.log('\t*** Conflict assigning', nextGuest, '--->', evt + '-' + hst, '\t(assignment skipped)');
					skipCnt++
				}}}
			}
		}
	...
	unDropHost(hostDrop: CdkDragDrop<any[]>, evt: string) {
		...
		if (fromHost === toHost) { moveItemInArray(hostDrop.container.data, hostDrop.previousIndex, hostDrop.currentIndex) } else { this.deAllocateHost(evt, hostDrop) }
	}
	unDropGuest(guestDrop: CdkDragDrop<any[]>, evt: string) {
		...
		const fromIdx	= guestDrop.previousIndex;
		if ( fromHst !== toHst ) {
			if (this.debug)  console.log( 'guest-plan-list moved:', fromHst, '--->', toHst );
	...
	deAllocateHost	(evt: string, hostDrop: CdkDragDrop<any[]>) {
		...
		if (this.debug)  console.log('\tChecking other events...:');
		for ( const e of this.events ) {
			if (this.debug)  console.log('\t\tevent:', e, 'host count:', this.unHs[e].length, '...');
			for ( const h in this.unHs[e] ) {
				if ( this.unHs[e][h].hostKey === hst ) {
					if (this.debug)  console.log('\t\t\thost#' + h, ':', this.unHs[e][h].hostKey, '\tdisable?\t', false);
	...
	allocateHost(evt: string, hst: string, prevHostIndex: number, hostDrop?: CdkDragDrop<any[]>) {
		if (this.debug) console.log('\tAllocating:', evt, hst, prevHostIndex, hostDrop);
		...
		this.aGs[evt][hst] = [];
		for ( const e of this.events ) { if ( e !== evt ) { for (const h in this.unHs[e]) { if (this.unHs[e][h].hostKey === hst)  this.unHs[e][h].isDisabled = true }}}
		...
		if (hst === gst) {
			...
			if (guestDrop) {
				...
				if (this.aGs[evt][hst].every((aG: {guestKey: string}) => this.summary.pairs[gst].includes(aG.guestKey))) {
					let fromHst:	string;
					let toIdx:		number;
					let fromIdx = this.unGs[evt].findIndex(guest => guest.guestKey === gst);
					if (fromIdx < 0) {
						////////////////////////////////////////////////////////////////////////////  host -> host		//
						for (const host of this.aGs[evt]) {
							fromHst	= host.hostKey;
							fromIdx	= this.aGs[evt][host.hostKey].findIndex(( guest: any) => guest.guestKey === gst );
							if (this.debug)  console.log('\tChecking' + host.hostKey, 'for:', gst);
							if (fromIdx > 0) {
								if (this.debug)  console.log('\t\tFOUND IT:', host.hostKey, '--->', fromIdx);
								toIdx = this.aGs[evt][hst].push(...this.aGs[evt][fromHst].splice( fromIdx, 1))
						...
						//////////////////////////////////////////////////////////////////////  unassigned -> host		//
						this.aGs[evt][hst].push(...this.unGs[evt].splice(fromIdx, 1))
	...
	rePair(evt: string, fromHost: string, toHost: string, gst: string) {
		if (this.debug)  console.log('rePair()\tevent:', evt, 'guest-plan-list:', gst, 'from:', fromHost, 'to:', toHost);
		const fromGuests	= this.aGs[evt][fromHost];
		const toGuests		= this.aGs[evt][toHost];
		if ( fromHost !== 'unassigned' ) {
			if (this.debug)  console.log('\tassignGuest()\tUn-pairing guests of:', fromHost + '...' );
			for ( const fromGuest of fromGuests ) {
				const fGst = fromGuest.guestKey;
				if (this.debug)  console.log('\t\t', gst, '--->', fGst);
				if (this.debug)  console.log('\t\t', gst, '<---', fGst);
		...
		if ( toHost !== 'unassigned' ) {
			if (this.debug)  console.log('\tassignGuest()\tRe-pairing guests of:', toHost + '...');
			for (const toGuest of toGuests) {
				const tGst = toGuest.guestKey;
				if (gst !== tGst) {
					if (this.debug)  console.log('\t\t', gst, '--->', tGst, 'idx:', this.summary.pairs[gst].indexOf(tGst));
					if (this.debug)  console.log('\t\t', gst, '<---', tGst, 'idx:', this.summary.pairs[gst].indexOf(gst));
		...
		for (const evt1 of events) {
			for (const hst1 of this.unHs[evt1]) {
				...
				for (const evt2 of events) {
					if (evt1 !== evt2) { for (const hst2 of this.unHs[evt2]) { if (searchObj.host === hst2.hostKey) {
